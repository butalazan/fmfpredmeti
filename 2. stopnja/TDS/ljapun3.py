...
for k in range(len(R)):
    ...
    ax2.hlines(y=avg(ljepunov(D, T)), xmin=len(D)/2, xmax=len(D), colors="black", linewidth=0.85, linestyles="dashed")
    ax2.text(305,avg(ljepunov(D, T)),s=f"$\lambda_\infty$={avg(ljepunov(D, T)):.2f}$\pm${sigma(ljepunov(D,T)):.2f}")
    ...
    #ax3.errorbar(cr, avg(ljepunov(D, T)), sigma(ljepunov(D,T)), fmt="o", ms=3, capsize=4, color="C0")
    ax3.scatter(cr, ljepunov(D, T)[-1], color="C0", s=9)
...
fig2.savefig("ljapun03b.png")
fig3.savefig("ljapun03c.png")