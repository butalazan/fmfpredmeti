...
fig1, ax1 = plt.subplots(tight_layout=True, figsize=(5.5,4.5))

y0s = np.linspace(-2.3,2.3,20)
for k in range(len(y0s)):
    sol_odeint = odeintegrator([-10, y0s[k], 0.7, 0., 0., 0.])
    ax1.plot(sol_odeint[0], sol_odeint[1], color="C0", lw=1.2)

x,y = np.linspace(-10,10,500), np.linspace(-10,10,500)
X,Y = np.meshgrid(x,y)
Z = RU(x-0.7,y-0.7,0)
CS = ax1.contour(X, Y, Z, 10, linewidths=0.6, colors="black")
...
fig2, ax2 = plt.subplots(tight_layout=True, figsize=(5.5,4.5))
from matplotlib.lines import Line2D
custom_lines = [Line2D([0], [0], color="C0", lw=1.8),
                Line2D([0], [0], color="C1", lw=1.8)]
ax2.legend(custom_lines, [r'$\varphi = 0$', r'$\varphi = \pi/8$'])

ax2.plot(sol_odeint[0], sol_odeint[1], label=r"$\varphi =0$")
ax2.plot(sol1[0], sol1[1], label=r"$\varphi =\pi/8$")
# ax2.plot(sol2[0], sol2[1])

x,y = np.linspace(-10,10,500), np.linspace(-10,10,500)
X,Y = np.meshgrid(x,y)
Z = RU(x-0.7,y-0.7,0)
CS = ax2.contour(X, Y, Z, 10, linewidths=0.6, colors="C0")
Z = RU(x-0.7,y-0.7,pi/8)
CS = ax2.contour(X, Y, Z, 10, linewidths=0.6, colors="C1")
Z = RU(x,y,pi/4)
