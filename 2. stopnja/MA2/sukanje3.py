...
M=10
sol_odeint_w = odeintegrator_zw([-10, 0.99, 0.7, 0., 0, 0, 0, 0, 0., 0.])
...
for j in range(0,len(t),2):
    ...
    #ax3.plot(sol_odeint[0][:j], sol_odeint[1][:j], "--", color="grey")
    ax3.plot(sol_odeint_w[0][:j], sol_odeint_w[1][:j], label=f"$v_0={sol_odeint_w[2][0]}$")
    ax3.scatter(sol_odeint_w[4][:j], sol_odeint_w[5][:j], color="C1", s=4, label=f"$m/M = 1/{M}$")
    ax3.axis("equal")
    ...
    fig3.savefig(f"./video/convert/oboje8_{1000+j}.png")