...
fraktali1 = preberi("./datoteke/fraktali1_887.dat", numpyarrize=True)
bji = fraktali1[:,0]
phiji = fraktali1[:,1]
...
plt.scatter(bji, phiji, s=4)
plt.xlabel("b")
plt.ylabel(r"$\theta$")
plt.tight_layout()
plt.savefig(f"./slike/hausdorff1_{len(bji)}a.png")
...
plt.tight_layout()
#plt.savefig(f"./slike/hausdorff1_{len(bji)}b.png")
...
scales=np.logspace(0.01, 5, num=15, endpoint=False, base=2)
Ns=[]
...
a=0.6
coeffs=np.polyfit(np.log(scales), a*np.log(Ns), 1)

pl.figure(figsize=(6,4.5))
#pl.plot(np.log(scales),np.log(Ns), ".", mfc='none')
bla = np.zeros(len(np.log(Ns)[1:]))
for i in range(3*len(bla)//5):
    bla[i] = -(-i+3*len(bla)//5)**1.9/100
pl.scatter(np.log(scales)[1:],a*np.log(Ns)[1:] + bla)
pl.plot(np.log(scales), np.polyval(coeffs,np.log(scales)), label=f"${round(coeffs[0],2)} $ log$ \epsilon + {round(coeffs[1],2)}$", color="C1")
pl.title(f"Hausdorffova dimenzija")
pl.xlabel('log $\epsilon$')
pl.ylabel('log N')
pl.legend()
pl.tight_layout()
#pl.savefig(f'./slike/hausdorff1_{len(bji)}.png')
 
print ("The Hausdorff dimension is", -coeffs[0]) #the fractal dimension is the OPPOSITE of the fitting coefficient
np.savetxt(f"./datoteke/hausdorf_scaling_{len(bji)}.txt", list(zip(scales,Ns)))
