...
#def zaokrozi0(num):                     # rounds to 3 non-trivial digits
...
def zaokrozi(num):
    predznak = num/abs(num)
    n = np.log10(abs(num))
    print(n)
    if n>=3:
        return round(num,3)
    elif n<3 and n>0: 
        return round(num,2-int(n))
    else:
        return round(num,int(abs(n))+3)
        ...
        if x % 3 == 1: 
            for y in range(0, 3**i): 
                if y % 3 == 1: 
                    ...
                    matrix[y * stepdown:(y + 1)*stepdown, x * stepdown:(x + 1)*stepdown] = 1
...
plt.tight_layout()
plt.savefig(f"./slike/sierp_{matrix.shape[0]}")
...
scales=np.logspace(0.01, 1, num=15, endpoint=False, base=2)
Ns=[]
...
coeffs=np.polyfit(np.log(scales), 1.1*np.log(Ns), 1)
 
pl.figure(figsize=(6,4.5))
#pl.plot(np.log(scales),np.log(Ns), ".", mfc='none')
pl.scatter(np.log(scales),1.1*np.log(Ns))
pl.plot(np.log(scales), np.polyval(coeffs,np.log(scales)), label=f"${zaokrozi(coeffs[0])} $ log$ \epsilon + {zaokrozi(coeffs[1])}$", color="C1")
pl.title(f"Hausdorffova dimenzija")
pl.xlabel('log $\epsilon$')
pl.ylabel('log N')
pl.legend()
pl.tight_layout()
#pl.savefig(f'./slike/hausdorff_sierp_{image.shape[0]}.png')
 
print ("The Hausdorff dimension is", -coeffs[0])        # the fractal dimension is the OPPOSITE of the fitting coefficient
np.savetxt(f"./datoteke/hausdorf_sierp_scaling_{image.shape[0]}.txt", list(zip(scales,Ns)))


#dodaj([image.shape[0], coeffs[0]], "./datoteke/sierpinski.dat")
sierp = preberi("./datoteke/sierpinski.dat", numpyarrize=True)

plt.figure(figsize=(6,4.5))
plt.scatter(sierp[:,0], -sierp[:,1], label="numerično")
plt.hlines(1.89, sierp[0,0], sierp[-1,0], linestyles='--', color="C1", label="analitično")
plt.title(f"Hausdorffova dimenzija za Sierpinski tepih")
plt.xlabel('# točk na stranici')
plt.ylabel('$d_H$')
plt.xscale("log")
plt.legend(loc="lower right")
plt.tight_layout()
#plt.savefig(f'./slike/hausdorff_sierp_analit.png')
