...
fig5 = plt.figure(figsize=(6.5,5.5))
ax5 = fig5.add_subplot(projection='3d')
x,y = np.linspace(-3,3,500), np.linspace(-3,3,500)
X,Y = np.meshgrid(x,y)
Z = RU(x,y,0)
ax5.plot_surface(X, Y, Z, linewidth = 0.2, antialiased = True, alpha=0.8, edgecolor="black")
ax5.set_xlabel('x')
ax5.set_ylabel('y')
ax5.set_zlabel('V(x,y)')
fig5.tight_layout()
...
for phi in Phiji:
    ...
    pocisti(f"./datoteke/sols2_{int(phi%1*1000)}.dat")
    pocisti(f"./datoteke/fraktali2_{int(phi%1*1000)}.dat")
    for a in range(len(bji)):

        sol_odeint = odeintegrator([-10, bji[a], 0.3, 0., phi, 0])
        thete[a] = kot(sol_odeint)
        # if a%50==0:
        #     plt.plot(sol_odeint[0], sol_odeint[1])
        #     plt.axis("equal")
        #     plt.show()
        dodaj(sol_odeint[0], f"./datoteke/sols2_{int(phi%1*1000)}.dat")
        dodaj(sol_odeint[1], f"./datoteke/sols2_{int(phi%1*1000)}.dat")
        dodaj([bji[a], thete[a], bji[1]-bji[0], phi], f"./datoteke/fraktali2_{int(phi%1*1000)}.dat")
